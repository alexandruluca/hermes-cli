...
async function downloadDeployment(deployment, {cwd, progress, unlinkIfExists} = {}) {
	...
	let deploymentBuffer = await api.deployments.downloadDeployment(deployment);
	let fileName = await api.deployments.getDeploymentName(deployment);
	console.log('fullDeploymentName', fileName);
	...
	try {
		fs.writeFileSync(tmpPath, deploymentBuffer)
